	...
	public static NativeImage composeFullMap(WorldAtlasData data, boolean markers, boolean zones, boolean routes, boolean labels) {
		...
		if (zones) {
			allMarkers.forEach((landmark, texture) -> {
				...
				net.minecraft.util.math.ColumnPos center = folk.sisby.roleplayers_atlas.util.TerritoryUtil.centroid(landmark.getOrDefault(LandmarkComponentTypes.CHUNKS, new HashMap<>()));
