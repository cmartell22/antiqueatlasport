...
public class ShareModal extends Component {
	...
	private class LayerRow extends ToggleButtonComponent {
		final folk.sisby.roleplayers_atlas.MarkerLayers.MapLayer layer;

		LayerRow(folk.sisby.roleplayers_atlas.MarkerLayers.MapLayer layer, int width) {
			super(true);
			this.layer = layer;
			setSize(width, ROW_H);
			addListener(button -> {
	...
	void setData(WorldSummary summary, DynamicRegistryManager manager, RegistryKey<World> dim, WorldAtlasData data) {
		...
		data.getEditableLandmarks().keySet().forEach(landmark -> {
			...
			String layer = folk.sisby.roleplayers_atlas.RoleplayersAtlas.layerOf(landmark);
			if (!folk.sisby.roleplayers_atlas.MarkerLayers.DEFAULT_ID.equals(layer)) {
				folk.sisby.roleplayers_atlas.MarkerLayers.MapLayer layerDef = folk.sisby.roleplayers_atlas.MarkerLayers.get(layer);
	...
	private void doExport() {
		...
		try {
			...
			if (player != null) {
				...
				player.sendMessage(Text.translatable("gui.roleplayers_atlas.share.exported", folk.sisby.roleplayers_atlas.ParchmentExport.folderLink(file)), false);
				folk.sisby.roleplayers_atlas.AtlasSounds.exportDone();
			}
			closeChild();
		} catch (Exception e) {
			folk.sisby.roleplayers_atlas.RoleplayersAtlas.LOGGER.warn("[Roleplayer's Atlas] Export failed", e);
	...
	public void render(DrawContext context, int mouseX, int mouseY, float partialTick) {
		...
		btnCorrections.active = !folk.sisby.roleplayers_atlas.BiomeOverrides.all().isEmpty()
			|| !folk.sisby.roleplayers_atlas.BiomeOverrides.ownPatches(dim).isEmpty()
			|| folk.sisby.roleplayers_atlas.CityPaint.ownCount(dim) > 0;
