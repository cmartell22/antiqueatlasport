...
public class MarkerModal extends Component {
	...
	void setMarkerData(WorldSummary summary, DynamicRegistryManager manager, Landmark baseLandmark) {
		...
		this.zoneTitleEnabled = !Boolean.FALSE.equals(baseLandmark.get(folk.sisby.roleplayers_atlas.AtlasComponents.ZONE_TITLE));
		Integer storedRadius = baseLandmark.get(folk.sisby.roleplayers_atlas.AtlasComponents.ZONE_RADIUS);
		this.zoneRadius = storedRadius != null ? storedRadius : folk.sisby.roleplayers_atlas.RoleplayersAtlas.CONFIG.zoneTitleRadius;
		Integer storedOpacity = baseLandmark.get(folk.sisby.roleplayers_atlas.AtlasComponents.OPACITY);
		this.markerOpacity = storedOpacity != null ? storedOpacity : 100;
		this.hideLabel = Boolean.TRUE.equals(baseLandmark.get(folk.sisby.roleplayers_atlas.AtlasComponents.HIDE_LABEL));
		this.markerLayer = baseLandmark.getOrDefault(folk.sisby.roleplayers_atlas.AtlasComponents.LAYER, "personal");
		this.noteText = baseLandmark.getOrDefault(folk.sisby.roleplayers_atlas.AtlasComponents.NOTE, "");
		this.dateEnabled = !Boolean.FALSE.equals(baseLandmark.get(folk.sisby.roleplayers_atlas.AtlasComponents.SHOW_DATE));
		this.showDistance = !Boolean.FALSE.equals(baseLandmark.get(folk.sisby.roleplayers_atlas.AtlasComponents.SHOW_DISTANCE));
		this.noteFocused = false;
		if (isSpawn()) {
			selectedTexture = folk.sisby.roleplayers_atlas.SpawnMarker.texture();
			selectedColor = folk.sisby.roleplayers_atlas.SpawnMarker.colorOf();
	...
	private Text layerText() {
		folk.sisby.roleplayers_atlas.MarkerLayers.MapLayer layer = folk.sisby.roleplayers_atlas.MarkerLayers.get(markerLayer);
	...
	protected boolean isPenLabel() {
		return baseLandmark != null && Boolean.TRUE.equals(baseLandmark.get(folk.sisby.roleplayers_atlas.AtlasComponents.PEN_LABEL));
	}

	/** Routes render as a dashed path — no icon, no zone settings. */
	protected boolean isRoute() {
		return baseLandmark != null && baseLandmark.contains(folk.sisby.roleplayers_atlas.AtlasComponents.ROUTE);
	}
	...
	protected boolean isSpawn() {
		return folk.sisby.roleplayers_atlas.SpawnMarker.is(baseLandmark);
	}
	...
	public void init() { // set up in here because it scales to parent size
		...
		addDrawableChild(btnDone = ButtonWidget.builder(Text.translatable("gui.done"), (button) -> {
			if (isSpawn()) {
				folk.sisby.roleplayers_atlas.RoleplayersAtlas.CONFIG.spawnMarkerIcon = selectedTexture.keyId().getPath().replaceFirst("^custom/", "");
				folk.sisby.roleplayers_atlas.RoleplayersAtlas.CONFIG.spawnMarkerColor = selectedColor.getId();
				folk.sisby.roleplayers_atlas.RoleplayersAtlas.CONFIG.spawnMarkerOpacity = markerOpacity;
				folk.sisby.roleplayers_atlas.RoleplayersAtlas.CONFIG.setAndSave("spawnMarkerIcon", folk.sisby.roleplayers_atlas.RoleplayersAtlas.CONFIG.spawnMarkerIcon);
				folk.sisby.roleplayers_atlas.RoleplayersAtlas.CONFIG.setAndSave("spawnMarkerColor", folk.sisby.roleplayers_atlas.RoleplayersAtlas.CONFIG.spawnMarkerColor);
				folk.sisby.roleplayers_atlas.RoleplayersAtlas.CONFIG.setAndSave("spawnMarkerOpacity", folk.sisby.roleplayers_atlas.RoleplayersAtlas.CONFIG.spawnMarkerOpacity);
			...
			if (landmarks != null) {
				...
				net.minecraft.util.math.ColumnPos territoryCenter = territory ? folk.sisby.roleplayers_atlas.util.TerritoryUtil.centroid(baseLandmark.getOrDefault(LandmarkComponentTypes.CHUNKS, new java.util.HashMap<>())) : null;
				Landmark written = WorldAtlasData.copyLandmarkWith(
					baseLandmark,
					territory
						? folk.sisby.roleplayers_atlas.RoleplayersAtlas.id("territory/" + territoryCenter.x() + "/" + territoryCenter.z() + "/" + Integer.toHexString(label.getString().hashCode()))
						: route
						? folk.sisby.roleplayers_atlas.RoleplayersAtlas.id("route/" + baseLandmark.get(LandmarkComponentTypes.POS).getX() + "/" + baseLandmark.get(LandmarkComponentTypes.POS).getZ() + "/" + Integer.toHexString(label.getString().hashCode()))
						: penLabel
						? folk.sisby.roleplayers_atlas.RoleplayersAtlas.id("label/" + baseLandmark.get(LandmarkComponentTypes.POS).getX() + "/" + baseLandmark.get(LandmarkComponentTypes.POS).getZ() + "/" + Integer.toHexString(label.getString().hashCode()))
					...
					if (!simple) {
						copy.set(folk.sisby.roleplayers_atlas.AtlasComponents.ZONE_TITLE, zoneTitleEnabled);
						copy.set(folk.sisby.roleplayers_atlas.AtlasComponents.ZONE_RADIUS, zoneRadius);
						copy.set(folk.sisby.roleplayers_atlas.AtlasComponents.HIDE_LABEL, hideLabel);
					}
					copy.set(folk.sisby.roleplayers_atlas.AtlasComponents.OPACITY, markerOpacity);
					copy.set(folk.sisby.roleplayers_atlas.AtlasComponents.LAYER, markerLayer);
					copy.set(folk.sisby.roleplayers_atlas.AtlasComponents.NOTE, noteText.trim());
					if (route) copy.set(folk.sisby.roleplayers_atlas.AtlasComponents.SHOW_DISTANCE, showDistance);
					// A mark keeps the date it was first drawn on; editing it later
					// doesn't rewrite history, it only decides whether to show it.
					copy.set(folk.sisby.roleplayers_atlas.AtlasComponents.SHOW_DATE, dateEnabled);
					Long drawnDay = baseLandmark.get(folk.sisby.roleplayers_atlas.AtlasComponents.DAY);
					copy.set(folk.sisby.roleplayers_atlas.AtlasComponents.DAY, drawnDay != null ? drawnDay : folk.sisby.roleplayers_atlas.AtlasTime.gameDay());
					Long drawnReal = baseLandmark.get(folk.sisby.roleplayers_atlas.AtlasComponents.REAL_TIME);
					copy.set(folk.sisby.roleplayers_atlas.AtlasComponents.REAL_TIME, drawnReal != null ? drawnReal : folk.sisby.roleplayers_atlas.AtlasTime.realMillis());
				});
		...
		if (isSpawn()) {
			...
			addDrawableChild(btnLayer = ButtonWidget.builder(layerText(), button -> {
				List<folk.sisby.roleplayers_atlas.MarkerLayers.MapLayer> allLayers = folk.sisby.roleplayers_atlas.MarkerLayers.all();
				int index = 0;
