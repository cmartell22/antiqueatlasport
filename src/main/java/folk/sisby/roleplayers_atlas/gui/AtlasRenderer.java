...
import folk.sisby.roleplayers_atlas.gui.tiles.SubTile;
import folk.sisby.roleplayers_atlas.gui.tiles.SubTileQuartet;
import folk.sisby.roleplayers_atlas.gui.tiles.TileRenderIterator;
...
import folk.sisby.roleplayers_atlas.util.DrawUtil;
import folk.sisby.roleplayers_atlas.util.MathUtil;
import folk.sisby.roleplayers_atlas.util.Rect;
...
public interface AtlasRenderer {
	...
	default float markerOpacity(Landmark landmark) {
		Integer opacity = landmark.get(folk.sisby.roleplayers_atlas.AtlasComponents.OPACITY);
		...
		if (folk.sisby.roleplayers_atlas.AtlasTime.isUnverified(landmark)) alpha *= 0.5F;
		return alpha;
	}
	...
	default void renderMarker(AtlasPainter painter, Landmark landmark, MarkerTexture texture, float z, int light, BiFunction<Double, Double, Float> alphaGetter, boolean pinned, boolean hovering, float markerScale) {
		...
		if (landmark.contains(folk.sisby.roleplayers_atlas.AtlasComponents.ROUTE)) {
			renderRoutePath(painter, landmark.get(folk.sisby.roleplayers_atlas.AtlasComponents.ROUTE), z, light,
		...
		if (Boolean.TRUE.equals(landmark.get(folk.sisby.roleplayers_atlas.AtlasComponents.PEN_LABEL))) return;
	...
	default void renderMarkerLabel(AtlasPainter painter, Landmark landmark, float labelAlpha, int light, float markerScale) {
		if (landmark.contains(folk.sisby.roleplayers_atlas.AtlasComponents.ROUTE)) return; // route names are written along the path
		if (landmark.owner() == null || landmark.owner().equals(folk.sisby.surveyor.landmark.WorldLandmarks.GLOBAL)) return;
		if (Boolean.TRUE.equals(landmark.get(folk.sisby.roleplayers_atlas.AtlasComponents.HIDE_LABEL))) return;
		...
		if (Boolean.TRUE.equals(landmark.get(folk.sisby.roleplayers_atlas.AtlasComponents.PEN_LABEL))) {
	...
	private java.util.List<double[]> sampleRoute(java.util.List<BlockPos> points, boolean straight) {
		...
		return folk.sisby.roleplayers_atlas.util.RouteUtil.sample(screen, straight);
	}
	...
	default void renderRouteLabel(AtlasPainter painter, Landmark landmark, float labelAlpha, int light) {
		...
		Text name = landmark.get(LandmarkComponentTypes.NAME);
		if (name == null || name.getString().isEmpty()) return;
		java.util.List<BlockPos> points = landmark.get(folk.sisby.roleplayers_atlas.AtlasComponents.ROUTE);
	...
	default void renderGuideArrows(AtlasPainter painter, float z, int light, float alpha, float scale) {
		...
		for (Map.Entry<Landmark, MarkerTexture> entry : worldAtlasData().getAllMarkers(tileChunks()).entrySet()) {
			...
			if (landmark.contains(folk.sisby.roleplayers_atlas.AtlasComponents.ROUTE)) {
				...
				net.minecraft.util.math.ColumnPos center = folk.sisby.roleplayers_atlas.util.TerritoryUtil.centroid(regions);
				tx = center.x();
				tz = center.z();
			}
			double dx = tx - px;
